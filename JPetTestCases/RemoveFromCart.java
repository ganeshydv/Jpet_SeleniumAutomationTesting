...
public class RemoveFromCart extends BaseClass{
	...
	//--------------------------EXTENT TEST REPORT ------------------
		...
		public static void startTest() {
			report = new ExtentReports(System.getProperty("user.dir") + "\\ExtentReports\\RemoveFromCartTestCaseExtentReportResults.html");
			test = report.startTest("Remove From Cart Test Case");
		...
		public void removeItemFromCartTest() 
		{
			...
			driver.manage().timeouts().implicitlyWait(500, TimeUnit.MILLISECONDS);
			WebElement tableRowsBefore=cart.cartBody();
			List<WebElement> lst=tableRowsBefore.findElements(By.xpath("*"));
			int itemsCounBefore=lst.size();
			System.out.println("size befor:"+lst.size());
			driver.manage().timeouts().implicitlyWait(1000,TimeUnit.MILLISECONDS);
			//------------------------- REMOVE FROM CART ------------------
			cart.removeItem().click();
			WebElement tableRowsAfter=cart.cartBody();
			List<WebElement> lstAfter=tableRowsAfter.findElements(By.xpath("*"));
			int itemsCounAfter=lstAfter.size();
			System.out.println("size after:"+lstAfter.size());
			if ((itemsCounAfter!=itemsCounBefore) || (itemsCounBefore==3)) {   //<<-------- Extent log
				test.log(LogStatus.PASS, "Remove item from cart test succesful");
			} else {
				test.log(LogStatus.FAIL, "Test Failed");
			}
			assertTrue((itemsCounAfter!=itemsCounBefore) || (itemsCounBefore==3),"Item not Removed");  // <<----- Assert validation
			System.out.println("Remove Item from cart successful....");
			driver.quit();
		}
